#!/usr/bin/env python3
"""
Chapter 1 - Solution A: Data-Driven MLP for Spring Oscillator

  python mlp_spring.py --epochs 1000

PDE: m*x'' + k*x = 0  (simple harmonic motion)
Training: generate 1000 (t, x) data points from analytical solution, MSE fit.
"""
...
# ── Physical parameters ───────────────────────────────────
M = 1.0  # mass (kg)
K = 4.0  # spring constant (N/m) -> omega = 2 rad/s
...
# ── main ──────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="MLP fit for spring oscillator")
    ...
    # Inference & visualization (extrapolate to 2*T_MAX)
    ...
    if mask_extrap.any():
        ...
        print(f"Extrapolation [{T_MAX}, {T_MAX*2}] MAE: {extrap_err:.4f}")

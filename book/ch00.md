...
> **难度**：导读（非技术章节）
> **本章目的**：在正文 7 章之前，说明为何学习 PhysicsNeMo 与 AI4Science
> **全书目录**：[SUMMARY.md](SUMMARY.md) · **体例**：[STYLE.md](STYLE.md)
...
某汽车主机厂的 NVH（噪声、振动）团队，要做一次悬挂调校扫参——50 个参数组合，每个跑一次完整 ANSYS Mechanical 模态分析。一次仿真 45 分钟，加上前后处理 1 小时，每个参数 1.75 小时。50 个参数串行跑，**87.5 小时**——一个工程师整整两周不能做别的。
...
- **坎 1（网格）**：神经算子可在**已离散**的表示上学习（结构网格、体素、点云等）；训练数据通常仍来自**已网格化的 CFD/FEM**。不是「完全不要网格」，而是**推理阶段**不必每次重画网格重跑求解器。
...
![F0.3 全书 7 章学习路线图](assets/f0_3_book_roadmap.png)
...
第 7 章  自定义实战 ────────── 汽车气动外形优化 / 端到端
   ↓
[附录 A 数学速查](appendix_a_math.md) / [附录 B 云 GPU](appendix_b_cloud_gpu.md) / [附录 C 踩坑 50 问](appendix_c_troubleshooting.md) / [附录 D PyTorch 最小集](appendix_d_pytorch_mini.md) / [附录 E 学完之后](appendix_e_further_learning.md)
```
...
**入门路径**：[START_HERE](../docs/START_HERE.md) · [CAE 工程师导读](../docs/START_HERE_CAE.md) · [第 1 天安装](../docs/QUICKSTART_DAY1.md) · [6 周计划](../docs/STUDY_PLAN_6WEEKS.md)
...
### 0.5.4 本书覆盖的物理场范围（CAE 读者）

| 领域 | 章节 | 本书深度 |
|:---|:---|:---|
| 结构 NVH / 模态 | ch00 故事、ch01 映射表 | 叙事类比，**无** ANSYS Mechanical 复现 |
| 导热 / 散热 | ch02–ch03 | 1D 瞬态 + **2D 稳态**散热片微缩 |
| 外流 CFD / 翼型 | ch04 | 叙事为翼型；**默认可跑为 Darcy**（见 CH04_GUIDE） |
| 多孔 / 混合物理 | ch05 | Darcy + 物理损失 |
| 天气时空场 | ch06 | toy 天气，非业务气象 |
| 汽车 $C_d$ 代理 + 优化 | ch07 | **toy 参数化车身**；DrivAerNet 为扩展档位 |

签审级结果仍需客户求解器抽检；见 [VV 报告模板](../docs/VV_REPORT_TEMPLATE.md)。
...
*前言 · 约 4,100 字 · 图表数：3 张 · 版本：v1.0 · 更新：2026-05-15*

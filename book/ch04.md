...
> **阅读时长**：约 45 分钟｜跑通代码约 30 分钟｜深入吃透约 2 小时
...
> **环境基线**：PhysicsNeMo v2.0 · PyTorch ≥ 2.3 · 8GB 显存跑 Darcy/FNO 微缩版；AirfRANS 完整版建议 24GB+ 或云 GPU
...
`<!-- IMAGE-TODO: docs/assets/ch04/airfoil_fno_banner.png -->`
`<!-- Gemini插画：工业风翼型绕流。左侧多种翼型轮廓，右侧彩色压力/速度云图，中间箭头标注 FNO surrogate。背景有傅里叶频谱纹理 -->`
...
1. **默认微缩路径**：先跑 PhysicsNeMo 官方风格的 Darcy/FNO 或 synthetic airfoil grid，8GB 显存可跑，目的是理解 FNO 训练流程。
2. **完整版路径**：接 AirfRANS 数据集，训练翼型 RANS 代理模型，建议 24GB+ 显存或云 GPU。

为什么不一上来就 AirfRANS？因为 AirfRANS 的数据准备比较重。教学上先跑通 FNO，再换真实翼型数据，学习曲线更稳。

### 4.2.1 微缩版：跑 Darcy/FNO fallback

```bash
cd ch04_fno_airfoil
python train_fno_mini.py dataset=darcy_mini epochs=50
```

预期输出：

```text
[INFO] PhysicsNeMo FNO mini training
[INFO] dataset=darcy_mini, resolution=64x64, train=1000, val=100
[INFO] model=FNO2D(modes=(12,12), width=32, layers=4)
epoch 000 | train_l2 8.42e-01 | val_l2 7.91e-01
epoch 010 | train_l2 1.83e-01 | val_l2 2.02e-01
epoch 050 | train_l2 3.91e-02 | val_l2 4.52e-02
[INFO] checkpoint saved: outputs/fno_mini/best.pt
```

### 4.2.2 翼型微缩版：synthetic airfoil grid

```bash
python train_fno_mini.py dataset=airfoil_synthetic epochs=80
python visualize_airfoil.py outputs/fno_airfoil_mini/best.pt
```

你会看到：输入是一张翼型 mask + 来流条件，输出是压力场/速度场的二维网格。

![F4.2 FNO 快速通道预测结果](assets/f4_2_fno_prediction.png)
`<!-- IMAGE-TODO: docs/assets/ch04/fno_mini_prediction.png -->`
`<!-- 实跑图：三列。输入翼型 mask；ground truth 压力场；FNO 预测压力场；右侧误差热图。发布前用真实训练结果替换 -->`
...
`<!-- IMAGE-TODO: docs/assets/ch04/pinn_vs_fno_cases.png -->`
`<!-- matplotlib图：横轴工况数 1,10,100,1000，纵轴总耗时（对数轴）。PINN 线性增长，FNO 前期训练成本高但后期推理成本低，交叉点在约 50-100 工况 -->`
...
`<!-- IMAGE-TODO: docs/assets/ch04/operator_mapping.png -->`
`<!-- Gemini插画：左侧是一张输入场（翼型mask/来流条件），中间是神经算子黑盒，右侧是压力场/速度场。上方标注 Gθ: a(x) -> u(x)。风格清晰，不要太科幻 -->`
...
`<!-- IMAGE-TODO: docs/assets/ch04/pressure_prediction_vs_truth.png -->`
`<!-- 实跑图：四列：input mask、ground truth pressure、FNO prediction、absolute error。选一个验证集翼型，色条统一 -->`
...
`<!-- IMAGE-TODO: docs/assets/ch04/cp_curve_comparison.png -->`
`<!-- 实跑图：横轴 x/c，纵轴 Cp（通常反向显示）。两条曲线：CFD ground truth 和 FNO prediction。上下表面用不同颜色或线型 -->`
...
`<!-- IMAGE-TODO: docs/assets/ch04/fno_tuning_summary.png -->`
`<!-- 实跑图：三联图。左：modes vs val_l2；中：width vs val_l2/显存；右：train_size vs val_l2。发布前用真实实验替换 -->`
...
在航空初设阶段，工程师需要快速扫大量翼型和攻角，找出升阻比高、失速特性好的候选。高保真 CFD 太慢，低保真工具又不够准。
...
*本章字数：约 11,800 字 · 图表数：10 张 · 完成日期：2026-05-15 · 版本：v1.0（W3 交付）*

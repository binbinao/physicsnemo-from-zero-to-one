...
一个 FNO block 把输入场 $x$ 经 **lifting 升维** 后，在 **频域** 做截断与谱卷积（Spectral Convolution），再 **iFFT** 回到空间域，最后接非线性激活与 pointwise 卷积，得到输出场（见图 F4.5；图内英文标签，频域区段为浅蓝底）。
...
## 4.6 🔵 AirfRANS 数据集：翼型 RANS 仿真样本（路径 B · 扩展）

> 📌 **与默认可跑代码的关系**：仓库默认训练是 **路径 A · Darcy**（`train_fno_mini.py`）。  
> 本节及 §4.7 的 Airfoil / AirfRANS 代码块属于 **路径 B 示例**，用于说明工业翼型数据长什么样；  
> **不要**期望默认 `train_fno_mini.py` 会加载这些翼型张量。路径对照见章首 T4.0 与 [CH04_GUIDE.md](../ch04_fno_airfoil/CH04_GUIDE.md)。
...
> 📌 **路径说明**：  
> - **路径 A（默认可跑）**：`train_fno_mini.py` + 合成 Darcy（`in_channels=1` 量级，见仓库脚本）。  
> - **路径 B（本节示例）**：下列 `in_channels=4` / `AirfoilTensorDataset` 为 **翼型 / AirfRANS 教学骨架**，需自备数据；不是 Day-1 默认入口。

### 4.7.1 模型定义（路径 B · 翼型示例骨架）

PhysicsNeMo 主框架提供 FNO 模型实现。API 可能随版本变化，但结构大致如下：

```python
"""路径 B 示例骨架（非默认 train_fno_mini.py）"""
import torch
from torch.utils.data import DataLoader
from physicsnemo.models.fno import FNO

model = FNO(
    in_channels=4,        # mask, sdf, aoa, reynolds（翼型特征）
...
> **路径 A**：对照仓库 `ch04_fno_airfoil/fno_model.py` + `train_fno_mini.py`（Darcy）。

### 4.7.2 数据加载（路径 B · 翼型示例）

```python
# 路径 B：需自备 airfrans_64；路径 A 请用 dataset.generate_darcy_data_fast()
...
## 4.12 🔵 延伸：DeepONet 与神经算子全景

FNO 并非唯一的神经算子范式。**DeepONet**（Lu et al. 2021, *Nature Machine Intelligence*）是另一条同样重要的主线，理论根基可追溯至万有算子逼近定理（Chen & Chen 1995）。
...
```text
Branch net：编码输入函数（如传感器读数）
Trunk net ：编码查询坐标（如时空位置）
输出 = Branch 输出 · Trunk 输出（内积）
```

Branch 负责"理解工况"，Trunk 负责"理解空间"；两者的点积就是算子在查询点的预测值。

### 4.12.2 FNO vs DeepONet 对比

| 维度 | FNO | DeepONet |
|---|---|---|
| 理论基础 | 频域卷积 (Green's function) | 万有算子逼近定理 (Chen & Chen 1995) |
| 网格要求 | 规则网格 (FFT) | 任意点云 / 非结构网格 |
| 输入形式 | 全场 tensor | 传感器离散采样 |
| 分辨率泛化 | 天然支持（频域截断） | 需显式设计 |
| 适用场景 | 规则域、大量相似工况 | 非结构域、稀疏观测 |

### 4.12.3 何时选哪个？

- **选 FNO**：问题定义在规则网格上、样本量充足、需要分辨率泛化（如气象、翼型规则域 CFD）。
- **选 DeepONet**：几何不规则或频繁变化、输入来自稀疏传感器、需要在任意查询点求值（如结构健康监测、实验数据驱动）。

实际项目中两者也可以组合：用 FNO 学大尺度场，用 DeepONet 在局部非结构区域做高精度补充。

### 4.12.4 近期扩展

- **POD-DeepONet**：用 POD 基函数加速 Trunk 收敛。
- **Physics-Informed DeepONet (PI-DeepONet)**：在损失中加入 PDE 残差，减少数据需求。
- **Multi-DeepONet**：多输出分支共享 Trunk，适合多物理场耦合。

> **经验法则**：如果你的数据已经在规则网格上，先试 FNO；如果你面对的是实验传感器数据或复杂非结构网格，DeepONet 可能是更自然的起点。

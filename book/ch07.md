...
> **📌 读前须知**：目录名 `ch07_drivaernet_optim` 以 **汽车 Cd 代理模型 + 优化 + 部署** 为完整示例（toy 合成车数据，非必须下载完整 DrivAerNet）。书中「自定义 PDE / Model」指**可替换为你自己的方程与网络**；方法论与 ch01–ch06 相同，换数据集即可迁移。

> **预计阅读**：正文约 45 分钟｜跑通代码约 1–2 小时｜深入吃透约半天
...
> **环境基线**：见 [ENVIRONMENT.md](../docs/ENVIRONMENT.md) · 8GB 显存可跑 synthetic car mini；DrivAerNet 子集建议 24GB+ 或云 GPU  
> **读法说明**：终章为端到端综合实战，不再分 🟢/🔵 快慢通道；按 7.0→7.11 顺序阅读即可。
...
我们用汽车气动做主线：**默认代码**用 toy 合成车参 + 公式 $C_d$ 跑通闭环；**扩展档位**可换 DrivAerNet 等真实 CFD 数据。我们训练代理模型预测 $C_d$（或压力场），优化器搜索更优设计，**再经高保真 CFD 复核**（`hifi_validation_queue.py`），最后可部署为 API。

> **多目标（C22）**：工业常同时优化 $C_d$、升力、俯仰力矩、风噪 proxy 等；本书仅演示 **单目标 $C_d$**，多目标可用 Optuna `multi-objective study` 扩展。
...
| 档位 | 数据 | 硬件 | 用途 |
|---|---|---|---|
| **toy（默认）** | `generate_toy_cars.py` 合成 $C_d$ | CPU/8GB | **仅流程演示**，不可对外签审 |
| mini | 真实 DrivAerNet 子集 100–500 样本 | 24GB | 训练代理模型 |
| full | DrivAerNet 全量/大子集 | A100/H100 | 认真 benchmark |

checkpoint `meta.data_tier` 会标注 `toy_synthetic`（见 `train.py`）。
...
### 7.6.1 高保真复核队列（C4 · 仓库脚本）

代理优化 **不能** 作为签审终点。训练/优化后请导出 Top-K 设计供 CFD 复核：
...
批跑骨架：`tools/cfd_batch/import_hifi_queue.py` + `run_batch.py --mode mock` → `results/cfd_runs/` → `ingest_results.py`（无需商业 CFD 许可证即可演示）。
...
### 7.6.4 一键闭环演示（mock CFD）

在仓库根目录（无需 Fluent/OpenFOAM 许可证）：
...
产出 `results/cfd_runs/validation_summary.csv`，可填入 [VV_REPORT_TEMPLATE](../docs/VV_REPORT_TEMPLATE.md)。详见 [CAE_CLOSED_LOOP_DEMO.md](../docs/CAE_CLOSED_LOOP_DEMO.md) · [CAE_OPTIMIZATION_CONSTRAINTS.md](../docs/CAE_OPTIMIZATION_CONSTRAINTS.md)。

流程说明：[CAE_DATA_GENERATION_SOP](../docs/CAE_DATA_GENERATION_SOP.md) · [tools/cfd_batch/README](../tools/cfd_batch/README.md) · [VV_REPORT_TEMPLATE](../docs/VV_REPORT_TEMPLATE.md)。

---

## 7.7 部署：FastAPI + Triton

> 生产差距见 [CAE_DEPLOYMENT_NOTES](../docs/CAE_DEPLOYMENT_NOTES.md)。
...
## 7.10 🔵 不确定性量化：代理模型的置信度

### 为什么需要 UQ？

工程签审需要回答的不只是"模型预测 $C_d=0.28$"，而是"$C_d$ 超过 0.30 的概率有多大？"。没有置信区间的预测，在安全关键场景中**等于没有预测**。

监管机构、主机厂、航空适航审查——任何签字画押的场合都需要不确定性量化（Uncertainty Quantification, UQ）。代理模型天生比高保真 CFD 多了一层模型误差；UQ 的目标就是**让这层误差可见、可量化、可决策**。

### 三种实用方法对比

| 方法 | 原理 | 实现难度 | 适用场景 |
|---|---|---|---|
| **Ensemble（集成）** | 训练 N 个不同初始化的模型，用预测标准差作为不确定性 | 低（N× 训练成本） | 通用，工业最常用 |
| **MC-Dropout** | 推理时保持 Dropout 开启，多次前向传播取均值和方差 | 极低（改一行代码） | 快速原型，轻量 UQ |
| **深度核学习 / GP** | 最后一层替换为高斯过程（Gaussian Process） | 中 | 小数据、需理论保证 |
...
UQ 最大的工程价值在于：**它可以驱动自动化验证闭环**。

当预测的不确定性 $\sigma_{C_d}$ 超过设定阈值时，自动将该设计点加入高保真复核队列：
...
这形成了一个**主动学习**循环：低置信度 → CFD 复核 → 新数据回流 → 模型增量训练 → 置信度提升。

### 本书未实现 UQ 的原因

1. **计算成本**：Ensemble 需要 5× 训练成本，教学环境下负担较大。
2. **教学聚焦**：本书目标是带你走通端到端基础流程，而非覆盖所有工业最佳实践。

> ⚠️ **强烈建议**：在实际项目中**必须**实施 UQ。没有不确定性估计的代理模型不应用于任何安全相关决策。
...
## 7.12 后记：不要相信任何没有验证闭环的 AI4Science demo
...
> - **微信公众号**：扫描下方二维码 ![微信公众号](../assets/wechat_qrcode.jpg) 关注
>
> ✅ **正文完结**：接下来请阅读 [附录 A 数学速查](appendix_a_math.md)、[附录 B 云 GPU 环境](appendix_b_cloud_gpu.md)、[附录 C 踩坑 50 问](appendix_c_troubleshooting.md)、[附录 D PyTorch 最小集](appendix_d_pytorch_mini.md)、[附录 E 学完之后：继续学习](appendix_e_further_learning.md)。  
> **下一步**：[附录 E](appendix_e_further_learning.md) · [读完后行动清单](../docs/WHATS_NEXT.md) · [6 周学习计划](../docs/STUDY_PLAN_6WEEKS.md)
...
*本章字数：约 10,900 字 · 图表数：7 张 · 版本：v1.0 · 更新：2026-05-15*

...
`<!-- IMAGE-TODO: docs/assets/ch07/drivaernet_solution_banner.png -->`
...
```bash
python train.py model=mlp data=toy_car training=debug
python train.py model=mlp data=drivaernet_mini training=full
```

### 7.6.3 优化命令

```bash
python optimize.py checkpoint=outputs/cd_mlp/best.pt n_trials=500
```
...
>
> ✅ **正文完结**：接下来请阅读 [附录 A 数学速查](appendix_a_math.md)、[附录 B 云 GPU 环境](appendix_b_cloud_gpu.md)、[附录 C 踩坑 50 问](appendix_c_troubleshooting.md)。

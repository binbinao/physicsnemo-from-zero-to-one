...
> **预计阅读**：正文约 40 分钟｜跑通代码约 20 分钟｜深入吃透约 2 小时
...
> **环境基线**：见 [ENVIRONMENT.md](../docs/ENVIRONMENT.md) · PhysicsNeMo v2.0 · PhysicsNeMo-Sym · PyTorch ≥ 2.3 · 8GB 显存可跑 2D three-fin 微缩版
>
> **读法建议**：先走 🟢 快速通道跑通 `heat_sink_train.py`。  
> **可选深读**（时间紧可后读）：§3.7 Validator、§3.8 反问题、§3.10 Failure 全表。
...
本章我们先做 **2D three-fin heat sink 微缩版**（平面导热，非 Icepak 全 3D 封装）。原因：8GB 显存能跑、能讲清多 BC 与 CSG。对标 **Icepak 3D 瞬态封装** 需另建网格与求解器数据管线（见 [CAE_DATA_GENERATION_SOP](../docs/CAE_DATA_GENERATION_SOP.md)）。
...
> **📌 裸 PyTorch 路径（`heat_sink_train.py`）**：权重键名为扁平的 `w_pde` / `w_bottom` / `w_side` / `w_robin`，见仓库 [`ch03_heatsink/conf/config.yaml`](../ch03_heatsink/conf/config.yaml)，与上文 Sym SDK 的 `loss_weights.*` 命名不同。
...
## 3.7 🔵 训练工程化：checkpoint / 日志 / validator

当问题进入工业几何，训练不再是"跑一个脚本看一张图"。你要能回答三个问题：

1. **训练到哪里了？** —— loss 曲线、约束项是否平衡。
2. **结果可靠吗？** —— validator 对比基准解（raw 路径见 `validator.py`；TensorBoard 为 SDK 可选能力，本书微缩脚本以日志 + JSON 报告为主）。
3. **中断能恢复吗？** —— checkpoint。
...
**本书仓库（raw PyTorch 路径）**：训练结束自动生成 `outputs/validation_report.json`（`ch03_heatsink/validator.py`：PDE/ Robin 残差 + 温度范围）。导入 Icepak/CFD 参考场后可扩展为 `PointwiseValidator` 对标。

**反问题（两种实现）**：

| 脚本 | 方法 |
|:---|:---|
| `heat_sink_inverse_joint.py` | **联合训练**：`fin_height` 与场变量同一优化循环（推荐） |
| `heat_sink_inverse.py` | 扫描法：每个高度重训 PINN（对照/demo） |

> **⚠️ 演示预期**：教学 PINN 在无量纲坐标下 tip 温度动态范围有限；`target_temp=40` 在 `--fast`（800 步）下**常无法严格达到**，仅验证「可微反演 + 早停」流程。签审请用 `validator.py` 与扫描版对照。详见 [CAE_CLOSED_LOOP_DEMO.md](../docs/CAE_CLOSED_LOOP_DEMO.md)。
...
*本章字数：约 11,100 字 · 图表数：10 张 · 版本：v1.0 · 更新：2026-05-15*

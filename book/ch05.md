...
> **预计阅读**：正文约 35 分钟｜跑通代码约 30 分钟｜深入吃透约 90 分钟
...
> **环境基线**：见 [ENVIRONMENT.md](../docs/ENVIRONMENT.md) · PhysicsNeMo v2.0 · PyTorch ≥ 2.3 · 8GB 显存可跑 64×64 Darcy 微缩版
...
![F5.0 多孔介质渗流横幅图](assets/f5_0_banner.png)
...
![F5.1 PINN/FNO/PINO 三角关系](assets/f5_1_triangle.png)
...
> **⚠️ 依赖第 4 章**：本章 import `ch04_fno_airfoil/fno_model.py`，请勿跳过 ch04 或单独拷贝本章目录。详见 [`ch05_darcy_hybrid/README.md`](../ch05_darcy_hybrid/README.md)。
...
![F5.2 快速通道：纯数据 FNO vs physics-informed FNO](assets/f5_2_comparison.png)
...
![F5.3 Darcy 物理示意图](assets/f5_3_darcy_physics.png)
...
![F5.4 Darcy 样本：渗透率场 → 压力场](assets/f5_4_darcy_samples.png)
...
> **CAE 说明（C10）**：`darcy_residual.py` 为 **均匀网格中心差分**，与 FEM 通量守恒格式不同；签审以 CFD/FEM 参考解为准。
...
> **版本提示（C28）**：正文 `PhysicsInformer` 为概念示例；**可运行实现**以 `ch05_darcy_hybrid/train_physics_fno.py` + `darcy_residual.py` 为准。import 路径以你安装的 PhysicsNeMo v2.x 为准。
...
![F5.5 小数据实验曲线](assets/f5_6_small_data.png)
...
![F5.6 λ_pde 扫描曲线](assets/f5_7_lambda_sweep.png)
...
![F5.7 多孔介质行业映射图](assets/f5_8_industry_map.png)
...
*本章字数：约 10,100 字 · 图表数：9 张 · 版本：v1.0 · 更新：2026-05-15*
